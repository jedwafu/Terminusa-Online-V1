...
setup_static_files() {
    ...
    sudo mkdir -p "$STATIC_DIR/css" || { error_log "Failed to create CSS directory"; return 1; }
    sudo mkdir -p "$STATIC_DIR/js" || { error_log "Failed to create JS directory"; return 1; }
    sudo mkdir -p "$STATIC_DIR/images" || { error_log "Failed to create images directory"; return 1; }
    ...
    if [ -d "$STATIC_DIR" ]; then
        ...
        sudo cp -r "$STATIC_DIR" "$BACKUP_DIR" || { error_log "Failed to create backup"; return 1; }
    fi
    
    # Copy files with error handling
    info_log "Copying static files..."
    sudo cp -r "./static/css/"* "$STATIC_DIR/css/" || { error_log "Failed to copy CSS files"; return 1; }
    sudo cp -r "./static/js/"* "$STATIC_DIR/js/" || { error_log "Failed to copy JS files"; return 1; }
    sudo cp -r "./static/images/"* "$STATIC_DIR/images/" 2>/dev/null || info_log "No images found"
    
    # Set permissions
    info_log "Setting permissions..."
    sudo chown -R www-data:www-data "$ROOT_DIR" || { error_log "Failed to set ownership"; return 1; }
    sudo chmod -R 755 "$ROOT_DIR" || { error_log "Failed to set permissions"; return 1; }
